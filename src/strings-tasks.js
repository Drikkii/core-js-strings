...
function removeLastOccurrences(str, value) {
  ...
  return result === -1
    ? str
    : str.slice(0, result) + str.slice(result + value.length);
}
...
function findLongestWord(sentence) {
  const slovo = sentence.split(' ');
  slovo.sort((a, b) => b.length - a.length);
  return slovo[0];
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function func(char) {
    const bill1 = char.charCodeAt(0);
    const bill2 = char <= 'Z' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
    const mega = ((bill1 - bill2 + 13) % 26) + bill2;

    return String.fromCharCode(mega);
  });
}
